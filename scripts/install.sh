...
show_banner() {
  ...
  if [[ "$cols" -ge 40 ]]; then
    ...
    if [[ -z "${NO_COLOR:-}" ]] && [[ -t 1 ]]; then
      ...
      c2=$'\033[38;2;255;200;120m'  # Light orange
    ...
    # Colorize each line left-to-right in 7-char zones
    _cl() {
      local line="$1" w=7
      printf '%s%s%s%s%s%s%s%s%s%s%s%s%s\n' \
        "$c1" "${line:0:w}" "$c2" "${line:w:w}" "$c3" "${line:$((w*2)):w}" \
        "$c4" "${line:$((w*3)):w}" "$c5" "${line:$((w*4)):w}" "$c6" "${line:$((w*5))}" "$r"
    }

    echo ""
    _cl '    __  ____                           __'
    _cl '   /  |/  (_)_______  ____  ____ _____/ /'
    _cl '  / /|_/ / / ___/ _ \/ __ \/ __ `/ __  /'
    _cl ' / /  / / / /__/  __/ /_/ / /_/ / /_/ /'
    _cl '/_/  /_/_/\___/\___/ .___/\__,_/\__,_/'
    printf '%s%s%s%s%s\n' "$c3" '                  /_/            ' "$b" 'CLI' "$r"
    echo ""
  else
    echo ""
